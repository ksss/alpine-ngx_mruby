...
ENV NGINX_CONFIG_OPT_ENV "\
  ...
  --with-http_v2_module \
  "
ENV NGINX_SRC_ENV /usr/src/nginx-$NGINX_VERSION
ENV NGX_MRUBY_VERSION 1.18.10
...
RUN apk add --no-cache --virtual nginx-build-deps \
    ...
    geoip-dev \
    perl-dev \
  && apk add --no-cache --virtual mruby-build-deps \
    git \
    git-subtree \
    make \
    ruby-rake \
...
RUN addgroup -S nginx \
  ...
  && rm nginx.tar.gz
### build ngx_mruby
RUN curl -fSL https://github.com/matsumotory/ngx_mruby/archive/v$NGX_MRUBY_VERSION.tar.gz -o ngx_mruby-$NGX_MRUBY_VERSION.tar.gz \
